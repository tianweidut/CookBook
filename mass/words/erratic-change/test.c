...
#define _IDX_NAME "source.idx"      //索引文件：只存原词
#define _DICT_NAME "source.dict"    //字典文件：只存解释
...
//获取偏移值
void *getWords(char *file,DICT_INFO *dict,WORD_IDX *idx);
//二分法搜索
...
int main(void)
{
  ...
  //  wordsum = FileSize;          
   ...
   // idx=(WORD_IDX*)malloc(sizeof((WORD_IDX*)(dict.wordcnt)));
   // WORD_IDX *idx=(WORD_IDX*)malloc(sizeof(WORD_IDX)*(WORDSUM));
    WORD_IDX idx[dict.wordcnt];
    ...
    getWords(filename,&dict,idx);   //索引重置
