...
class Parse():
    def run(self):
        ...
        """
        parse = OptionParser()
        parse.add_option("-o", "--openHadoop",
                         action="store_true", dest="is_hadoop",
                         help="whether it is running on hadoop platform")
        parse.add_option("-c", "--closeHadoop",
                         action="store_false", dest="is_hadoop",
                         help="whether it is running on hadoop platform")
        parse.add_option("-n", "--num",
                         action="store", dest="num",
                         help="num for elsvm")
        parse.add_option("-v", "--varibles",
                         action="store", dest="var",
                         help="v for elsvm")
        parse.add_option("-s", "--sampleName",
                         action="store", dest="sample_name",
                         default="elsvm_data.csv", help="sample for elsvm")
        parse.add_option("-p", "--path",
                         action="store", dest="path",
                         help="sample data path")
        parse.add_option("-t", "--output_path",
                         action="store", dest="output_path",
                         help="result output path")
        parse.add_option("-l", "--local_path",
                         action="store", dest="local_path",
                         help="local output path")
        parse.add_option("-m", "--output_name",
                         action="store",
                         dest="output_name",
                         help="output name")
        parse.add_option("-d", "--dim",
                         action="store", dest="dim",
                         help="dim number for sample data")
        parse.add_option("-i", "--enableIncrement",
                         action="store_true", dest="is_increment",
